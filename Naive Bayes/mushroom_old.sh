...
if [ "x$alg" != "xclean" ]; then
  ...
  mkdir -p ${WORK_DIR}
  if [ ! -e ${WORK_DIR}/mushroom-bayesinput ]; then
    if [ ! -e ${WORK_DIR}/mushroom-bydate ]; then

      mkdir -p ${WORK_DIR}/mushroom-bydate/training
      mkdir -p ${WORK_DIR}/mushroom-bydate/test
      echo "Extracting..."
      cp -R ${START_PATH}/data/mushroom.training ${WORK_DIR}/mushroom-bydate/training && cp -R ${START_PATH}/data/mushroom.test ${WORK_DIR}/mushroom-bydate/test && cd .. && cd ..
    fi
  fi
fi
#echo $START_PATH
...
if  ( [ "x$alg" == "xnaivebayes-MapReduce" ] ||  [ "x$alg" == "xcnaivebayes-MapReduce" ]); then
  ...
  rm -rf ${WORK_DIR}/mushroom-all
  mkdir ${WORK_DIR}/mushroom-all
  cp -R ${WORK_DIR}/mushroom-bydate/* ${WORK_DIR}/mushroom-all

  if [ "$HADOOP_HOME" != "" ] && [ "$MAHOUT_LOCAL" == "" ] ; then
    echo "Copying 20newsgroups data to HDFS"
    set +e
    $DFSRM ${WORK_DIR}/mushroom-all
    $DFS -mkdir ${WORK_DIR}
    $DFS -mkdir ${WORK_DIR}/mushroom-all
    set -e
    if [ $HVERSION -eq "1" ] ; then
      echo "Copying 20newsgroups data to Hadoop 1 HDFS"
      $DFS -put ${WORK_DIR}/mushroom-all ${WORK_DIR}/mushroom-all
    elif [ $HVERSION -eq "2" ] ; then
      echo "Copying 20newsgroups data to Hadoop 2 HDFS"
      $DFS -put ${WORK_DIR}/mushroom-all ${WORK_DIR}/
    fi
  fi

  echo "Creating sequence files from 20newsgroups data"
  ./bin/mahout seqdirectory \
    -i ${WORK_DIR}/mushroom-all/training/ \
    -o ${WORK_DIR}/mushroom-seq/training -ow

  ./bin/mahout seqdirectory \
    -i ${WORK_DIR}/mushroom-all/test/ \
    -o ${WORK_DIR}/mushroom-seq/test -ow

  echo "Converting sequence files to vectors"
  ./bin/mahout seq2sparse \
    -i ${WORK_DIR}/mushroom-seq/training/ \
    -o ${WORK_DIR}/mushroom-vectors/training 

  ./bin/mahout seq2sparse \
    -i ${WORK_DIR}/mushroom-seq/test/ \
    -o ${WORK_DIR}/mushroom-vectors/test 
